...
def train_via_dih(student,
    ...
    :param traind_branch_classifers_dict:  a dictionary of mounted trained intermediate classifier heads
    ...
    :param multiple_gpu: for check wether multi gpus are available
    ...
    for (branch_key, branch_classifer) in traind_branch_classifers_dict.items():
        branch_classifer.to(device)
        branch_classifer.eval()

    paralleled_branch_classifer_dict = {}
    if "cuda" in device_to_train_on:
        if multiple_gpu is not None:
            ...
            for (branch_key,branch_classifer) in traind_branch_classifers_dict.items():
                paralleled_branch_classifer_dict[branch_key] = nn.DataParallel(branch_classifer, device_ids=multiple_gpu)
                paralleled_branch_classifer_dict[branch_key].eval()
        else:
            print("MultiGPU is inactive")

            paralleled_branch_classifer_dict = traind_branch_classifers_dict

    since = time.time()
    student.to(device)

    data_loader_dict, dataset_sizes = get_train_valid_loader_cifars(output_height=input_sample_size[2],
                                                                    output_width=input_sample_size[1],
                                                                    batch_size=input_sample_size[0],
                                                                    cifar10_100=dataset,
                                                                    augment=True)

    test_loader = get_test_loader_cifar(output_height=input_sample_size[1],
                                        output_width=input_sample_size[2],
                                        batch_size=input_sample_size[0],
                                        dataset=dataset)
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in data_loader_dict[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    for (branch_key,branch_classifer) in paralleled_branch_classifer_dict.items():
                        #for Non VGG Models
                        if not isinstance(teacher_outputs[1],list):
                            out_d_dict[branch_key] = branch_classifer(teacher_outputs[branch_key])
                            complete_teacher_outputs_dict[branch_key] =  branch_classifer(teacher_outputs[branch_key])
                        else:
                            out_d_dict[branch_key] = branch_classifer(teacher_outputs[1][branch_key-1])
                            complete_teacher_outputs_dict[branch_key] = branch_classifer(teacher_outputs[1][branch_key-1])
    ...
    #_,_ = test_data_evaluation(student,
                         #     test_loader,
